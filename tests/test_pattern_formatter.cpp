...
static std::string log_to_str(const std::string &msg, const Args &...args) {
    ...
    xlog::logger oss_logger("pattern_tester", oss_sink);
    oss_logger.set_level(xlog::level::info);

    oss_logger.set_formatter(
        std::unique_ptr<xlog::formatter>(new xlog::pattern_formatter(args...)));
...
template <typename... Args>
static std::string log_to_str_with_time(xlog::log_clock::time_point log_time,
    ...
    oss_logger.log(log_time, {}, xlog::level::info, msg);
    return oss.str();
}
...
TEST_CASE("custom eol", "[pattern_formatter]") {
    ...
    std::string eol = ";)";
    REQUIRE(log_to_str(msg, "%v", xlog::pattern_time_type::local, ";)") == msg + eol);
}

TEST_CASE("empty format", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "", xlog::pattern_time_type::local, "").empty());
}

TEST_CASE("empty format2", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "", xlog::pattern_time_type::local, "\n") == "\n");
}

TEST_CASE("level", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "[%l] %v", xlog::pattern_time_type::local, "\n") ==
            "[info] Some message\n");
}
...
TEST_CASE("name", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "[%n] %v", xlog::pattern_time_type::local, "\n") ==
            "[pattern_tester] Some message\n");
}

TEST_CASE("date MM/DD/YY ", "[pattern_formatter]") {
    auto now_tm = xlog::details::os::localtime();
    std::stringstream oss;
    oss << std::setfill('0') << std::setw(2) << now_tm.tm_mon + 1 << "/" << std::setw(2)
        << now_tm.tm_mday << "/" << std::setw(2) << (now_tm.tm_year + 1900) % 1000
        << " Some message\n";
    REQUIRE(log_to_str("Some message", "%D %v", xlog::pattern_time_type::local, "\n") ==
            oss.str());
}
...
TEST_CASE("UTC offset", "[pattern_formatter]") {
    ...
    std::string result =
        log_to_str_with_time(now, "Some message", "%z", xlog::pattern_time_type::local, "\n");
...
TEST_CASE("color range test1", "[pattern_formatter]") {
    auto formatter = std::make_shared<xlog::pattern_formatter>(
        "%^%v%$", xlog::pattern_time_type::local, "\n");

    memory_buf_t buf;
    xlog::fmt_lib::format_to(std::back_inserter(buf), "Hello");
    memory_buf_t formatted;
    std::string logger_name = "test";
    xlog::details::log_msg msg(logger_name, xlog::level::info,
                                 xlog::string_view_t(buf.data(), buf.size()));
    formatter->format(msg, formatted);
    REQUIRE(msg.color_range_start == 0);
    REQUIRE(msg.color_range_end == 5);
    REQUIRE(log_to_str("hello", "%^%v%$", xlog::pattern_time_type::local, "\n") == "hello\n");
}

TEST_CASE("color range test2", "[pattern_formatter]") {
    auto formatter =
        std::make_shared<xlog::pattern_formatter>("%^%$", xlog::pattern_time_type::local, "\n");
    std::string logger_name = "test";
    xlog::details::log_msg msg(logger_name, xlog::level::info, "");
    memory_buf_t formatted;
    formatter->format(msg, formatted);
    ...
    REQUIRE(log_to_str("", "%^%$", xlog::pattern_time_type::local, "\n") == "\n");
}

TEST_CASE("color range test3", "[pattern_formatter]") {
    auto formatter = std::make_shared<xlog::pattern_formatter>("%^***%$");
    std::string logger_name = "test";
    xlog::details::log_msg msg(logger_name, xlog::level::info, "ignored");
    memory_buf_t formatted;
    formatter->format(msg, formatted);
    REQUIRE(msg.color_range_start == 0);
...
TEST_CASE("color range test4", "[pattern_formatter]") {
    auto formatter = std::make_shared<xlog::pattern_formatter>(
        "XX%^YYY%$", xlog::pattern_time_type::local, "\n");
    std::string logger_name = "test";
    xlog::details::log_msg msg(logger_name, xlog::level::info, "ignored");
    ...
    REQUIRE(msg.color_range_end == 5);
    REQUIRE(log_to_str("ignored", "XX%^YYY%$", xlog::pattern_time_type::local, "\n") ==
            "XXYYY\n");
}
...
TEST_CASE("level_left_padded", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "[%8l] %v", xlog::pattern_time_type::local, "\n") ==
            "[    info] Some message\n");
    REQUIRE(log_to_str("Some message", "[%8!l] %v", xlog::pattern_time_type::local, "\n") ==
            "[    info] Some message\n");
}
...
TEST_CASE("short level_left_padded", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "[%3L] %v", xlog::pattern_time_type::local, "\n") ==
            "[  I] Some message\n");
    REQUIRE(log_to_str("Some message", "[%3!L] %v", xlog::pattern_time_type::local, "\n") ==
            "[  I] Some message\n");
}
...
TEST_CASE("left_padded_short", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "[%3n] %v", xlog::pattern_time_type::local, "\n") ==
            "[pattern_tester] Some message\n");
    REQUIRE(log_to_str("Some message", "[%3!n] %v", xlog::pattern_time_type::local, "\n") ==
            "[pat] Some message\n");
}
...
TEST_CASE("left_padded_huge", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "[%-300n] %v", xlog::pattern_time_type::local, "\n") ==
            "[pattern_tester                                                  ] Some message\n");

    REQUIRE(log_to_str("Some message", "[%-300!n] %v", xlog::pattern_time_type::local, "\n") ==
            "[pattern_tester                                                  ] Some message\n");
}

TEST_CASE("left_padded_max", "[pattern_formatter]") {
    REQUIRE(log_to_str("Some message", "[%-64n] %v", xlog::pattern_time_type::local, "\n") ==
            "[pattern_tester                                                  ] Some message\n");

    REQUIRE(log_to_str("Some message", "[%-64!n] %v", xlog::pattern_time_type::local, "\n") ==
            "[pattern_tester                                                  ] Some message\n");
}
...
TEST_CASE("paddinng_truncate", "[pattern_formatter]") {
    REQUIRE(log_to_str("123456", "%6!v", xlog::pattern_time_type::local, "\n") == "123456\n");
    REQUIRE(log_to_str("123456", "%5!v", xlog::pattern_time_type::local, "\n") == "12345\n");
    REQUIRE(log_to_str("123456", "%7!v", xlog::pattern_time_type::local, "\n") == " 123456\n");
    ...
    REQUIRE(log_to_str("123456", "%0!v", xlog::pattern_time_type::local, "\n") == "\n");
}

TEST_CASE("padding_truncate_funcname", "[pattern_formatter]") {
    xlog::sinks::test_sink_st test_sink;

    const char *pattern = "%v [%5!!]";
    auto formatter = std::unique_ptr<xlog::formatter>(new xlog::pattern_formatter(pattern));
    test_sink.set_formatter(std::move(formatter));

    xlog::details::log_msg msg1{xlog::source_loc{"ignored", 1, "func"}, "test_logger",
                                  xlog::level::info, "message"};
    test_sink.log(msg1);
    REQUIRE(test_sink.lines()[0] == "message [ func]");

    xlog::details::log_msg msg2{xlog::source_loc{"ignored", 1, "function"}, "test_logger",
                                  xlog::level::info, "message"};
    test_sink.log(msg2);
...
TEST_CASE("padding_funcname", "[pattern_formatter]") {
    ...
    xlog::details::log_msg msg2{xlog::source_loc{"ignored", 1, "func567890123"}, "test_logger",
                                  xlog::level::info, "message"};
    test_sink.log(msg2);
...
TEST_CASE("clone-default-formatter", "[pattern_formatter]") {
    auto formatter_1 = std::make_shared<xlog::pattern_formatter>();
    auto formatter_2 = formatter_1->clone();
    std::string logger_name = "test";
    xlog::details::log_msg msg(logger_name, xlog::level::info, "some message");
...
TEST_CASE("clone-formatter", "[pattern_formatter]") {
    auto formatter_1 = std::make_shared<xlog::pattern_formatter>("%D %X [%] [%n] %v");
    auto formatter_2 = formatter_1->clone();
    std::string logger_name = "test";
    xlog::details::log_msg msg(logger_name, xlog::level::info, "some message");
...
class custom_test_flag : public xlog::custom_flag_formatter {
public:
    explicit custom_test_flag(std::string txt)
        : some_txt{std::move(txt)} {}

    void format(const xlog::details::log_msg &,
                const std::tm &tm,
                xlog::memory_buf_t &dest) override {
        if (some_txt == "throw_me") {
            throw xlog::spdlog_ex("custom_flag_exception_test");
        } else if (some_txt == "time") {
            auto formatted = xlog::fmt_lib::format("{:d}:{:02d}{:s}", tm.tm_hour % 12, tm.tm_min,
                                                     tm.tm_hour / 12 ? "PM" : "AM");
    ...
    xlog::details::padding_info get_padding_info() { return padinfo_; }

    std::string some_txt;

    std::unique_ptr<custom_flag_formatter> clone() const override {
        return xlog::details::make_unique<custom_test_flag>(some_txt);
    }
};
...
TEST_CASE("clone-custom_formatter", "[pattern_formatter]") {
    ...
    auto expected = xlog::fmt_lib::format("[logger-name] [custom_output] some message{}",
                                            xlog::details::os::default_eol);
...
TEST_CASE("short filename formatter-1", "[pattern_formatter]") {
    xlog::pattern_formatter formatter("%s", xlog::pattern_time_type::local, "");
    memory_buf_t formatted;
    std::string logger_name = "logger-name";
    xlog::source_loc source_loc{test_path, 123, "some_func()"};
    xlog::details::log_msg msg(source_loc, "logger-name", xlog::level::info, "Hello");
    formatter.format(msg, formatted);
...
TEST_CASE("short filename formatter-2", "[pattern_formatter]") {
    ...
    memory_buf_t formatted;
    std::string logger_name = "logger-name";
    xlog::source_loc source_loc{"myfile.cpp", 123, "some_func()"};
    xlog::details::log_msg msg(source_loc, "logger-name", xlog::level::info, "Hello");
    formatter.format(msg, formatted);
...
TEST_CASE("short filename formatter-3", "[pattern_formatter]") {
    xlog::pattern_formatter formatter("%s %v", xlog::pattern_time_type::local, "");
    memory_buf_t formatted;
    std::string logger_name = "logger-name";
    xlog::source_loc source_loc{"", 123, "some_func()"};
    xlog::details::log_msg msg(source_loc, "logger-name", xlog::level::info, "Hello");
    formatter.format(msg, formatted);
...
TEST_CASE("full filename formatter", "[pattern_formatter]") {
    xlog::pattern_formatter formatter("%g", xlog::pattern_time_type::local, "");
    memory_buf_t formatted;
    std::string logger_name = "logger-name";
    xlog::source_loc source_loc{test_path, 123, "some_func()"};
    xlog::details::log_msg msg(source_loc, "logger-name", xlog::level::info, "Hello");
...
TEST_CASE("custom flags", "[pattern_formatter]") {
    ...
    xlog::details::log_msg msg(xlog::source_loc{}, "logger-name", xlog::level::info,
                                 "some message");
    formatter->format(msg, formatted);
    auto expected = xlog::fmt_lib::format("[logger-name] [custom1] [custom2] some message{}",
                                            xlog::details::os::default_eol);
...
TEST_CASE("custom flags-exception", "[pattern_formatter]") {
    ...
    xlog::details::log_msg msg(xlog::source_loc{}, "logger-name", xlog::level::info,
                                 "some message");
    CHECK_THROWS_AS(formatter->format(msg, formatted), xlog::spdlog_ex);
}
...
TEST_CASE("mdc formatter test-1", "[pattern_formatter]") {
    ...
    xlog::mdc::put("mdc_key_2", "mdc_value_2");
...
TEST_CASE("mdc formatter value update", "[pattern_formatter]") {
    ...
    xlog::mdc::put("mdc_key_1", "new_mdc_value_1");
    memory_buf_t formatted_2;
    formatter->format(msg, formatted_2);
    expected = xlog::fmt_lib::format(
...
TEST_CASE("mdc different threads", "[pattern_formatter]") {
    ...
    auto lambda_1 = [formatter, msg]() {
        xlog::mdc::put("mdc_key", "thread_1_id");
        memory_buf_t formatted;
        formatter->format(msg, formatted);

        auto expected =
            xlog::fmt_lib::format("[logger-name] [info] [mdc_key:thread_1_id] some message{}",
                                    xlog::details::os::default_eol);
    ...
    auto lambda_2 = [formatter, msg]() {
        xlog::mdc::put("mdc_key", "thread_2_id");
        memory_buf_t formatted;
        formatter->format(msg, formatted);

        auto expected =
            xlog::fmt_lib::format("[logger-name] [info] [mdc_key:thread_2_id] some message{}",
                                    xlog::details::os::default_eol);
...
TEST_CASE("mdc remove key", "[pattern_formatter]") {
    ...
    auto expected =
        xlog::fmt_lib::format("[logger-name] [info] [mdc_key_2:mdc_value_2] some message{}",
                                xlog::details::os::default_eol);
    REQUIRE(to_string_view(formatted) == expected);
...
TEST_CASE("mdc empty", "[pattern_formatter]") {
    ...
    auto expected = xlog::fmt_lib::format("[logger-name] [info] [] some message{}",
                                            xlog::details::os::default_eol);
    REQUIRE(to_string_view(formatted) == expected);
