...
TEST_CASE("simple_file_logger", "[simple_logger]") {
    ...
    REQUIRE(file_contents(SIMPLE_LOG) ==
            xlog::fmt_lib::format("Test message 1{}Test message 2{}", default_eol, default_eol));
}
...
TEST_CASE("flush_on", "[flush_on]") {
    ...
    using xlog::details::os::default_eol;
    REQUIRE(file_contents(SIMPLE_LOG) ==
            xlog::fmt_lib::format("Should not be flushed{}Test message 1{}Test message 2{}",
                                    default_eol, default_eol, default_eol));
}

TEST_CASE("simple_file_logger", "[truncate]") {
    prepare_logdir();
    const xlog::filename_t filename = XLOG_FILENAME_T(SIMPLE_LOG);
    const bool truncate = true;
    const auto sink = std::make_shared<xlog::sinks::basic_file_sink_mt>(filename, truncate);
    const auto logger = std::make_shared<xlog::logger>("simple_file_logger", sink);
...
TEST_CASE("rotating_file_logger1", "[rotating_logger]") {
    ...
    xlog::filename_t basename = XLOG_FILENAME_T(ROTATING_LOG);
    auto logger = xlog::rotating_logger_mt("logger", basename, max_size, 0);
...
TEST_CASE("rotating_file_logger3", "[rotating_logger]") {
    ...
    size_t max_size = 0;
    xlog::filename_t basename = XLOG_FILENAME_T(ROTATING_LOG);
    REQUIRE_THROWS_AS(xlog::rotating_logger_mt("logger", basename, max_size, 0),
                      xlog::spdlog_ex);
}
...
TEST_CASE("rotating_file_logger4", "[rotating_logger]") {
    ...
    size_t max_size = 1024 * 10;
    xlog::filename_t basename = XLOG_FILENAME_T(ROTATING_LOG);
    auto sink = std::make_shared<xlog::sinks::rotating_file_sink_st>(basename, max_size, 2);
    auto logger = std::make_shared<xlog::logger>("rotating_sink_logger", sink);
...
TEST_CASE("rotating_file_logger5", "[rotating_logger]") {
    ...
    xlog::filename_t basename = XLOG_FILENAME_T(ROTATING_LOG);
    auto sink =
        std::make_shared<xlog::sinks::rotating_file_sink_st>(basename, max_size, max_files);
    auto logger = std::make_shared<xlog::logger>("rotating_sink_logger", sink);
    logger->set_pattern("%v");
