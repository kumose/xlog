...
| `TVLOG` / `DTVLOG` | fmt verbose INFO (same gates as `XVLOG`) |
| `ZVLOG` / `DZVLOG` | printf verbose INFO (same gates as `XVLOG`) |
...
`XVLOG` / `TVLOG` / `ZVLOG` share `EVERY_N` / `FIRST_N` / `ONCE` / `EVERY_POW_2` /
`EVERY_N_SEC` (and `D*` variants).
